//
// Copyright note: Redistribution and use in source, with or without modification, are permitted.
// 
// Created: October 2018
// 
// @author:  Johan Falk
// SICK AG, Waldkirch
// email: TechSupport0905@sick.de
// 
// Last commit: $Date: 2017-09-27 10:44:05 +0200 (Mi, 27 Sep 2017) $
// Last editor: $Author: falkjo $
// 
// Version "$Revision: 14104 $"
//

#pragma once

#include <vector>
#include <stdint.h>

#include "CoLaCommandType.h"
#include "CoLaBCommand.h"

/// <summary>
/// Builder for constructing <see cref="CoLaBCommand" />s.
/// </summary>
class CoLaBCommandBuilder
{
private:
  CoLaCommandType::Enum m_type;
  const char* m_name;
  std::vector<uint8_t> m_buffer;

public:
  /// <summary>
  /// Construct a new <see cref="CoLaBCommandBuilder" />.
  /// </summary>
  /// <param name="type">Type of command.</param>
  /// <param name="command">The command, e.g. for methods this should be the "communication name" from the CID.</param>
  CoLaBCommandBuilder(CoLaCommandType::Enum type, const char* name);
  ~CoLaBCommandBuilder();

  /// <summary>
  /// Add a signed short (8-bit, range [-128, 127]).
  /// </summary>
  /// <param name="sInt">SInt to add</param>
  /// <returns>This builder.</returns>
  CoLaBCommandBuilder& parameterSInt(const int8_t sInt);

  /// <summary>
  /// Add a unsigned short (8-bit, range [0, 255]).
  /// </summary>
  /// <param name="uSInt">USInt to add.</param>
  /// <returns>This builder.</returns>
  CoLaBCommandBuilder& parameterUSInt(const uint8_t uSInt);

  /// <summary>
  /// Add a signed int (16-bit).
  /// </summary>
  /// <param name="integer">Int to add.</param>
  /// <returns>This builder.</returns>
  CoLaBCommandBuilder& parameterInt(const int16_t integer);

  /// <summary>
  /// Add a unsigned int (16-bit, range [0, 65535]).
  /// </summary>
  /// <param name="uInt">UInt to add.</param>
  /// <returns>This builder.</returns>
  CoLaBCommandBuilder& parameterUInt(const uint16_t uInt);

  /// <summary>
  /// Add an signed double int (32-bit).
  /// </summary>
  /// <param name="dInt">dInt to add.</param>
  /// <returns>This builder.</returns>
  CoLaBCommandBuilder& parameterDInt(const int32_t dInt);

  /// <summary>
  /// Add an unsigned double int (32-bit, range [0, 4294967295]).
  /// </summary>
  /// <param name="uDInt">UDInt to add.</param>
  /// <returns>This builder.</returns>
  CoLaBCommandBuilder& parameterUDInt(const uint32_t uDInt);

  /// <summary>
  /// Add a IEEE-754 single precision (32-bit).
  /// </summary>
  /// <param name="real">Real to add.</param>
  /// <returns>This builder.</returns>
  CoLaBCommandBuilder& parameterReal(const float real);

  /// <summary>
  /// Add a IEEE-754 double precision (64-bit).
  /// </summary>
  /// <param name="lReal">Long real to add.</param>
  /// <returns>This builder.</returns>
  CoLaBCommandBuilder& parameterLReal(const double lReal);

  /// <summary>
  /// Add a boolean as a byte, with 0 representing false, and 1 representing true.
  /// </summary>
  /// <param name="boolean">Boolean to add.</param>
  /// <returns>This builder.</returns>
  CoLaBCommandBuilder& parameterBool(const bool boolean);

  /// <summary>
  /// Add the given password as a UDInt, using MD5 hashing.
  /// </summary>
  /// <param name="str">String to hash and add as parameter.</param>
  /// <returns>This builder.</returns>
  CoLaBCommandBuilder& parameterPasswordMD5(const char* str);

  /// <summary>
  /// Add a string parameter, must be null-terminated.
  /// </summary>
  /// <param name="str">String to add.</param>
  /// <returns>This builder.</returns>
  CoLaBCommandBuilder& operator<<(const char* str);

  /// <summary>
  /// Add a signed short (8-bit, range [-128, 127]).
  /// </summary>
  /// <param name="sInt">SInt to add</param>
  /// <returns>This builder.</returns>
  CoLaBCommandBuilder& operator<<(const int8_t sInt);

  /// <summary>
  /// Add a unsigned short (8-bit, range [0, 255]).
  /// </summary>
  /// <param name="uSInt">USInt to add.</param>
  /// <returns>This builder.</returns>
  CoLaBCommandBuilder& operator<<(const uint8_t uSInt);

  /// <summary>
  /// Add a signed int (16-bit).
  /// </summary>
  /// <param name="integer">Int to add.</param>
  /// <returns>This builder.</returns>
  CoLaBCommandBuilder& operator<<(const int16_t integer);

  /// <summary>
  /// Add a unsigned int (16-bit, range [0, 65535]).
  /// </summary>
  /// <param name="uInt">UInt to add.</param>
  /// <returns>This builder.</returns>
  CoLaBCommandBuilder& operator<<(const uint16_t uInt);

  /// <summary>
  /// Add an signed double int (32-bit).
  /// </summary>
  /// <param name="dInt">dInt to add.</param>
  /// <returns>This builder.</returns>
  CoLaBCommandBuilder& operator<<(const int32_t dInt);

  /// <summary>
  /// Add an unsigned double int (32-bit, range [0, 4294967295]).
  /// </summary>
  /// <param name="uDInt">UDInt to add.</param>
  /// <returns>This builder.</returns>
  CoLaBCommandBuilder& operator<<(const uint32_t uDInt);

  /// <summary>
  /// Add a IEEE-754 single precision (32-bit).
  /// </summary>
  /// <param name="real">Real to add.</param>
  /// <returns>This builder.</returns>
  CoLaBCommandBuilder& operator<<(const float real);

  /// <summary>
  /// Add a IEEE-754 double precision (64-bit).
  /// </summary>
  /// <param name="lReal">Long real to add.</param>
  /// <returns>This builder.</returns>
  CoLaBCommandBuilder& operator<<(const double lReal);

  /// <summary>
  /// Add a boolean as a byte, with 0 representing false, and 1 representing true.
  /// </summary>
  /// <param name="boolean">Boolean to add.</param>
  /// <returns>This builder.</returns>
  CoLaBCommandBuilder& operator<<(const bool boolean);

  const CoLaBCommand build();

private:
  void writeHeader(CoLaCommandType::Enum type, const char* name);

  /// <summary>
  /// Calculates the checksum for the given buffer.
  /// </summary>
  /// <param name="buffer">Buffer to calculate checksum for.</param>
  /// <returns>The calculated checksum.</returns>
  static uint8_t calculateChecksum(const std::vector<uint8_t>& buffer);
};